...
import com.example.demo9.form.ReviewRegistForm;
import com.example.demo9.service.RegistService;
...
public class ReviewController {
    ...
    public String confirmRegistReview(
            @Validated @ModelAttribute ReviewRegistForm form, // @ModelAttributeを追加
            BindingResult result,
            Model model) {
        ...
        Review r = new Review();
        // 以下のセッター名をEntityの定義に合わせて修正
        r.setRestaurantId(form.getRestaurantId());
        r.setUserId(form.getUserId());        // LocalsetUserId -> setUserId に修正
        r.setVisitDate(form.getVisitDate());   // Date -> setVisitDate に修正
